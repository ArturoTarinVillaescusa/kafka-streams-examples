package io.confluent.examples.streams.streamdsl.processorapi.transformvalues;

import io.confluent.common.utils.TestUtils;
import io.confluent.examples.streams.streamdsl.processorapi.utils.transformers.EmailMetricThresholdAlert_ValueTransformerSupplier;
...
/**
 * DSL Transform Values API integration using State Store:
 * =======================================================
 *
 ...
 *
 * 1) The first strategy is to manually add the StoreBuilders via
 ...
 *
 *
 * 2) The second strategy (DEMOSTRATED IN THIS EXAMPLE) is for the given TransformerSupplier to implement
 *    ConnectedStoreProvider.stores(), which provides the StoreBuilders to be automatically added to the
 *    topology and connected to the transformer.
 **/

@Slf4j
public class O02_transformValuesStrategy2_ImplementTransformerSupplier {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        KStream transformedStream =
                sourceStream.transformValues(new EmailMetricThresholdAlert_ValueTransformerSupplier());
