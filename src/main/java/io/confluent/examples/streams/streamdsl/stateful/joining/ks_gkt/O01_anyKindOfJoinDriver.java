...
import static io.confluent.examples.streams.streamdsl.stateful.joining.ks_gkt.O01_anyKindOfJoin.*;

/**
 * This is a sample driver for the {@link O01_anyKindOfJoin}.
 * To run this driver please first refer to the instructions in {@link O01_anyKindOfJoin}.
 ...
 * <pre>
 * {@code
 * $ java -cp target/kafka-streams-examples-6.0.0-standalone.jar io.confluent.examples.streams.streamsdsl.stateful.joining.ks_gkt.O01_anyKindOfJoinDriver
 * }
 * </pre>
 */
public class O01_anyKindOfJoinDriver {
  ...
  public static List<Customer> generateCustomers(
    ...
    final String [] firstnames = {"john", "doe", "franklin", "chandar", "elisabeth", "peter", "james", "clint"};
    final String [] lastnames = {"ford", "johanson", "doe", "stephenson", "pal", "trout", "fenderman", "mckee"};
    final String [] nationalities = {"spanish", "english", "greek", "italian", "chipriot"};
    final boolean [] vips = {true, false};
    final Random random = new Random();
    for(long i = 0; i < count; i++) {
      final Customer customer = new Customer(firstnames[random.nextInt(firstnames.length)],
                                            lastnames[random.nextInt(lastnames.length)],
                                            random.nextInt(120),
                                            genders[random.nextInt(genders.length)],
                                            vips[random.nextInt(vips.length)],
