...
/**
 * Demonstrating the KStream DSL `aggregate` method applied to a KTable
 *
 **/

public class O03_aggregateTable {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        // Key: username, value: user region (abreviated to "E" for Europe, "A" for Asia)
        KStream<String, String> userProfilesStream = streamsBuilder.stream(inputTopic,
                Consumed.with(Serdes.String(), Serdes.String()));

        // Create a table from the stream
        KTable<String, String> userProfilesKTable = userProfilesStream.toTable();

        KGroupedTable<String, Integer> userProfilesGroupedKTable =
                userProfilesKTable.groupBy((user, region) -> KeyValue.pair(region, user.length())
                );

        KTable<String, Long> aggregatedTable = userProfilesGroupedKTable.aggregate(
                () -> 0L,                                            // Initializer
                (aggKey, newValue, aggValue) -> aggValue + newValue,// Aggregator
                ...
                Materialized.<String, Long, KeyValueStore<Bytes, byte[]>>as("aggregated-table-store") // State Store
        ...
        aggregatedTable.toStream().to(aggTableOutputTopic);
