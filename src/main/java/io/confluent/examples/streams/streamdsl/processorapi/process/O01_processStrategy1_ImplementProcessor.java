...
import io.confluent.examples.streams.streamdsl.processorapi.utils.processors.EmailMetricThresholdAlert_Processor;
 ...
 *
 * 1) the first strategy (DEMONSTRATED IN THIS EXAMPLE) is to manually add the StoreBuilder
...
@Slf4j
public class O01_processStrategy1_ImplementProcessor {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        sourceStream.process(new ProcessorSupplier() {
            @Override
        ...
        // Alternatively
//        sourceStream
//                .process(() -> new EmailMetricThresholdAlert_Processor(), stateStoreName);

        sourceStream.print(Printed.toFile("/tmp/pageViewsAfterApplyingTheFilterProcessor.txt"));

        sourceStream.to(outputTopic, Produced.as("DESTINATION-STREAM"));
    }
