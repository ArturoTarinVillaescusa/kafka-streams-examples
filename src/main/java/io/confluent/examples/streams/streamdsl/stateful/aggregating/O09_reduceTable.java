...
public class O09_reduceTable {
    static final String inputTopic = "input";
    static final String aggTableOutputTopic = "reduce-table-output";
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        KTable<String, Long> table = stream.toTable();

        KGroupedTable<String, Long> userProfilesGroupedKTable =
                table.groupBy((k, v) -> KeyValue.pair(k, v)
                );

        KTable<String, Long> aggregatedTable = userProfilesGroupedKTable.reduce(
                ...
                Materialized.<String, Long, KeyValueStore<Bytes, byte[]>>as("reduce-table-store") // State Store
