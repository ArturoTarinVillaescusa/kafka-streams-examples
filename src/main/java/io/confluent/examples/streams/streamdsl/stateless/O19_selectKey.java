...
/**
 * Demonstrates the use of the `selectKey` high-level KStream DSL
 * selectKey derives a new record key from the record's value.  Note how the key type changes, too.
 **/

public class O19_selectKey {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        // selectKey derives a new record key from the record's value.
        // Note how the key type changes, too.
        // This example will use every first word of the value sentence as a key in the resulting stream
        // i.e: (A@3B453, "Phrase to be rekeyed") -> ("Phrase", "Phrase to be rekeyed")
        KStream<String, String> reKeyedStream = stream.selectKey((key, value) -> value.split(" ")[0]);

        // Write the mapped values to the output topic
        reKeyedStream.to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
    }
