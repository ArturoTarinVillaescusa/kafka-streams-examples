...
/**
 * Demonstrates, using the high-level KStream DSL, how to create a KTable from an input topic
 **/

public class O2_KTableFromTopic {
    static final String inputTopic = "streams-plaintext-input";
    static final String outputTopic = "table-plaintext-output";
    ...
    public static void createStream(final StreamsBuilder streamsBuilder) {
        ...
        final KTable<String, String> table = streamsBuilder.table(inputTopic,
                Consumed.with(Serdes.String(), Serdes.String()));

        // In this early example we are not going to apply any topology transformation, we
        // just leave the messages as they are

        // Write the KTable<String, String> to the output topic
        table.toStream().to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
    }
    ...
    public static Properties getStreamsConfiguration(final String bootstrapServers) {
        ...
        properties.put(StreamsConfig.APPLICATION_ID_CONFIG, "ktable-from-topic");
        properties.put(StreamsConfig.CLIENT_ID_CONFIG, "ktable-from-topic-client");
