...
/**
 * Demonstrating the KStream DSL `count` method applied to a KTable
 *
 **/

public class O06_countTable {
    static final String inputTopic = "input";
    static final String countStreamOutputTopic = "aggregated-stream-output";
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        KStream<String, Integer> wordCounts = streamsBuilder.stream(inputTopic,
                Consumed.with(Serdes.String(), Serdes.Integer()));
        // Group the stream
        KGroupedStream<String, Integer> groupedStream =
                wordCounts.groupByKey(Grouped.with(Serdes.String(), Serdes.Integer()));

        // Create a table from the stream
        KTable<String, Integer> wordCountsKTable = wordCounts.toTable();

        KGroupedTable<String, Integer> groupedTable =
            wordCountsKTable.groupBy((word, count) -> KeyValue.pair(word, count)
        );

        // Count the groupedTable
        KTable<String, Long> countTable = groupedTable.count();

        countTable.toStream().to(countTableOutputTopic);
    }
