...
/**
 * Demonstrates the use of the `print` high-level KStream DSL
 **/

public class O17_print {
    public static final String inputTopic1 = "streams-input-1";
    public static final String inputTopic2 = "streams-input-2";
    public static final String outputTopic = "streams-merge-output";
    ...
    public static void createStream(final StreamsBuilder streamsBuilder) {
        ...
        final KStream<byte[], String> stream1 = streamsBuilder.stream(inputTopic1,
                Consumed.with(Serdes.ByteArray(), Serdes.String()));

        final KStream<byte[], String> stream2 = streamsBuilder.stream(inputTopic2,
                Consumed.with(Serdes.ByteArray(), Serdes.String()));

        final KStream<byte[], String> mergedStreams = stream1.merge(stream2);

        mergedStreams.print(Printed.toSysOut());
        mergedStreams.print(Printed.toFile("/tmp/print-stream-example.txt"));

        // Write the mapped values to the output topic
        mergedStreams.to(outputTopic, Produced.with(Serdes.ByteArray(), Serdes.String()));
    }
