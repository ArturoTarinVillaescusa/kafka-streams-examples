...
/**
 * Demonstrates the use of the `flatmapValues` high-level KStream DSL
 * Will not cause data re-partitioning. Use `flatMap` instead if you need to
 * modify keys or key type
 **/

public class O8_flatMapValues {
    public static final String inputTopic = "streams-input";
    public static final String outputTopic = "streams-flatmapvalues-output";
    ...
    public static void createStream(final StreamsBuilder streamsBuilder) {
        ...
        // apply a flatMapValue transformation to the values. Our example is splitting sentences
        // into words
        final KStream<byte[], String> words = stream.flatMapValues(
                // Example: (1, "Hello World!") -> (1, "Hello"), (1, "World!")
                value -> Arrays.asList(value.split("\\s+")));

        // Write the flatMapped transformed values to the output topic
        words.to(outputTopic, Produced.with(Serdes.ByteArray(), Serdes.String()));
    }
    ...
    public static Properties getStreamsConfiguration(final String bootstrapServers) {
        ...
        properties.put(StreamsConfig.APPLICATION_ID_CONFIG, "kstreams-flatmapvalues");
        properties.put(StreamsConfig.CLIENT_ID_CONFIG, "kstreams-flatmapvalues-client");
