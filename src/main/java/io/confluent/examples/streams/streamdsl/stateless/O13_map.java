...
/**
 * Demonstrates the use of the `map` high-level KStream DSL
...
public class O13_map {
    public static final String inputTopic = "streams-input";
    public static final String outputTopic = "streams-map-output";
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        // apply a map transformation to the stream topology.
        // Marks the stream for data re-partitioning: Applying a grouping or
        // a join after map will result in re-partitioning of the records.
        // If possible use mapValues instead, which will not cause data re-partitioning.
        KStream<String, Integer> mappedValues = stream.map(
                // Here, for each input record we generate an output record changing
                // the key, the value and their types too.
                // Example: ("Hello", "World!") -> ("WORLD!", 5)
                (key, value) -> KeyValue.pair(value.toUpperCase(), key.length)
        );

        // Write the mapped values to the output topic
        mappedValues.to(outputTopic, Produced.with(Serdes.String(), Serdes.Integer()));
    }
