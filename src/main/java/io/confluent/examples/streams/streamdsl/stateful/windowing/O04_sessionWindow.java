...
public class O04_sessionWindow {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        KTable<Windowed<String>, Long> sessionizedPageViewCounts =
                pageViews.groupByKey(Grouped.with(Serdes.String(), new GenericAvroSerde()))
                         .windowedBy(SessionWindows.with(Duration.ofMinutes(5)))
                        .count();

        sessionizedPageViewCounts.toStream().print(Printed.toFile("/tmp/windowedPageViewCounts.txt"));

        sessionizedPageViewCounts.toStream().to(countTableOutputTopic);
    }
