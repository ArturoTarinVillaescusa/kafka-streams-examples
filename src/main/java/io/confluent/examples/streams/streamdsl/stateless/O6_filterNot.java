...
/**
 * Demonstrates the use of the `filterNot` high-level KStream DSL
 **/

public class O6_filterNot {
    public static final String inputTopic = "streams-input";
    public static final String outputTopic = "streams-greater-than-200-output";
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        // apply a filter transformation to the stream topology in order to get only values > 200
        KStream<String, Long> onlyGreaterThan200 = stream.filterNot(
                (key, value) -> value <= 200L
        );

        // Write the greater values  to the output topic
        onlyGreaterThan200.to(outputTopic, Produced.with(Serdes.String(), Serdes.Long()));
    }
    ...
    public static Properties getStreamsConfiguration(String bootstrapServers) {
        ...
        properties.put(StreamsConfig.APPLICATION_ID_CONFIG, "kstreams-filter-not");
        properties.put(StreamsConfig.CLIENT_ID_CONFIG, "kstreams-filter-not-client");
