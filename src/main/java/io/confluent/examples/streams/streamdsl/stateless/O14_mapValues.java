...
/**
 * Demonstrates the use of the `mapValues` high-level KStream DSL
 * DOESN'T mark the stream for data re-partitioning.
 **/

public class O14_mapValues {
    public static final String inputTopic = "streams-input";
    public static final String outputTopic = "streams-mapvalues-output";
    ...
    public static void createStream(final StreamsBuilder streamsBuilder) {
        ...
        // apply a mapValues transformation to the stream topology.
        // When possible use mapValues because will not cause data re-partitioning.
        final KStream<byte[], Integer> mappedValues = stream.mapValues(
                // Here, for each input record we generate an output record changing
                // the value and value type.
                // The key remains intact, but we can use it in read-only mode
                // Example: ("Hello", "World!") -> ("Hello", 11)
                (k, v) -> k.length + v.length()
        );

        // Write the mapped values to the output topic
        mappedValues.to(outputTopic, Produced.with(Serdes.ByteArray(), Serdes.Integer()));
    }
