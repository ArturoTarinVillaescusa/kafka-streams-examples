...
/**
 * Demonstrating the KStream DSL `count` method applied to a KStream
 *
 **/

public class O05_countStream {
    ...
    public static void createStream(StreamsBuilder streamsBuilder) {
        ...
        // Count the groupedStream
        KTable<String, Long> countStream = groupedStream.count();

        countStream.toStream().print(Printed.toSysOut());

        // Write the countStream to the output topic
        countStream.toStream().to(countStreamOutputTopic);
    }
