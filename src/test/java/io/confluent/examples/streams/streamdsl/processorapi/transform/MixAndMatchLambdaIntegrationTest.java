 ...
 * method to include a custom {@link Transformer} (from the
...
public class MixAndMatchLambdaIntegrationTest {
  ...
  public void shouldAnonymizeTheInput() {
    ...
    final KStream<byte[], String> stream = builder.stream(inputTopic, Consumed.as("SOURCE-STREAM"));
    final KStream<byte[], String> uppercasedAndAnonymized = stream
      .mapValues(s -> s.toUpperCase(), Named.as("MAP-STREAM-VALUES-TO-UPPERCASE"))
      .transform(AnonymizeIpAddressTransformer::new, Named.as("TRANSFORM-THE-STREAM"));
    uppercasedAndAnonymized.to(outputTopic, Produced.as("DESTINATION-STREAM"));
