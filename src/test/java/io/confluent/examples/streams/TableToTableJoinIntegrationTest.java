...
public class TableToTableJoinIntegrationTest {
  ...
  public void shouldJoinTwoTables() {
    ...
    final KTable<String, Long> userLastLogins = builder.table(userLastLoginTopic,
            Consumed.with(stringSerde, longSerde));

    userRegions.toStream().print(Printed.toFile("/tmp/userRegions.txt"));
    userLastLogins.toStream().print(Printed.toFile("/tmp/userLastLogins.txt"));

    final String storeName = "joined-store";
    KStream<String, String> joinedStore = userRegions.join(userLastLogins,
            (regionValue, lastLoginValue) -> regionValue + "/" + lastLoginValue,
            Materialized.as(storeName))
            .toStream();
    joinedStore.print(Printed.toFile("/tmp/joinedStore.txt"));

    joinedStore.to(outputTopic, Produced.with(Serdes.String(), Serdes.String()));
    ...
    System.out.println("\n||||||||||||||||||\n\n" + topology.describe() +
            ...
            "If you save the image then you can open it in Chrome url " +
